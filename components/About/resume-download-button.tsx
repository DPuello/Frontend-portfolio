...
export const ResumeDownloadButton = () => {
  ...
  const handleClientSideError = () => {
    console.error(
      "PDF generation failed in client, switching to server-side generation",
    );
  ...
  const translationFunc = (key: string) => {
    try {
      ...
      if (
        !key.includes("resumeContent") &&
        !actualKey.includes("resumeContent")
      ) {
      ...
      if (actualKey === "resumeContent.education.title") {
        ...
        const nestedResult = t(
          "resumeContent.experience.education.title",
          undefined,
          true,
        );

        if (nestedResult && typeof nestedResult === "string")
          return nestedResult;
      ...
      if (actualKey === "resumeContent.education.items") {
        ...
        // Try nested path as fallback
        const nestedResult = t(
          "resumeContent.experience.education.items",
          undefined,
          true,
        );
      ...
      if (actualKey === "resumeContent.projects.title") {
        ...
        // Try nested path as fallback
        const nestedResult = t(
          "resumeContent.experience.education.projects.title",
          undefined,
          true,
        );
      ...
      if (actualKey === "resumeContent.projects.items") {
        ...
        // Try nested path as fallback
        const nestedResult = t(
          "resumeContent.experience.education.projects.items",
          undefined,
          true,
        );
