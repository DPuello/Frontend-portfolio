...
function Shape({
  ...
  const mobilePositionY = isMobile
    ? position[1] * PERFORMANCE_CONFIG.MOBILE.POSITION_SCALE_Y
    : position[1];
  const mobileSize = isMobile
    ? size * PERFORMANCE_CONFIG.MOBILE.SIZE_MULTIPLIER
    : size;
  ...
  const shouldLimitFramerate =
    PERFORMANCE_CONFIG.FRAME_RATE.LIMIT_FRAME_RATE &&
    (!PERFORMANCE_CONFIG.FRAME_RATE.ONLY_LIMIT_ON_LOWER_PERFORMANCE ||
      performanceLevel !== "high");
  const shouldRenderFrame = useFrameLimit(
    PERFORMANCE_CONFIG.FRAME_RATE.TARGET_FPS,
  ...
  useFrame((state, delta) => {
    ...
    const maxInfluenceRadius = isMobile
      ? PERFORMANCE_CONFIG.MOBILE.TOUCH_INFLUENCE_RADIUS
      : 0.4; // Radius in NDC space (-1 to 1)
    ...
    const currentSpeed =
      baseSpeed.current *
      THREE.MathUtils.lerp(1, skipComplexEffects ? 2 : 10.5, influenceFactor) *
      speedMultiplier;
    ...
    if (!skipComplexEffects) {
      ...
      const movementAmplitude =
        THREE.MathUtils.lerp(0.05, 0.25, influenceFactor) * speedMultiplier;
      const movementFrequency = 1.5 * speedMultiplier;
      // Calculate target position offset based on sine waves
      const targetX =
        targetPosition[0] +
        Math.sin(time * movementFrequency + targetPosition[0]) *
          movementAmplitude;
      const targetY =
        targetPosition[1] +
        Math.cos(time * movementFrequency * 0.8 + targetPosition[1]) *
          movementAmplitude;
      const targetZ =
        targetPosition[2] +
        Math.sin(time * movementFrequency * 0.6 + targetPosition[2]) *
          movementAmplitude *
          0.5;
      ...
      meshRef.current.position.x =
        targetPosition[0] + Math.sin(time * 0.5) * 0.05;
      meshRef.current.position.y =
        targetPosition[1] + Math.cos(time * 0.4) * 0.05;
    }

    // --- Scale --- (Simplified for low performance)
    if (!skipComplexEffects) {
      const pulseScale = 1 + Math.sin(state.clock.elapsedTime * 1.5) * 0.05;
      const hoverScaleMultiplier = THREE.MathUtils.lerp(
        1,
        1.4,
        influenceFactor,
      );
      const targetScaleValue =
        mobileSize * hoverScaleMultiplier * (active ? 1.5 : 1) * pulseScale;

      meshRef.current.scale.lerp(
        new THREE.Vector3(targetScaleValue, targetScaleValue, targetScaleValue),
        0.1,
      );
    }
    ...
    if (!skipComplexEffects) {
      currentMaterialColor.lerpColors(
        baseColor,
        targetHoverColor,
        influenceFactor,
      );
      ...
      const themeEmissiveBoost =
        state.scene.userData.theme === "light" ? 2.0 : 1.2;
  ...
  const ShapeComponent = () => {
    switch (shape) {
      ...
      case "sphere":
        ...
        const sphereDetail =
          complexity === "low" || performanceLevel === "low"
            ? 8
            : complexity === "medium" || performanceLevel === "medium"
              ? 12
              : 24;
      ...
      case "torus":
        ...
        return (
          <torusGeometry
            args={[0.7, 0.3, torusRadialSegments, torusTubularSegments]}
          />
        );
      default:
...
function PerformanceMonitor({
  ...
  const isFramerateLimited = useRef<boolean>(
    PERFORMANCE_CONFIG.FRAME_RATE.LIMIT_FRAME_RATE,
  );
  ...
  useFrame((state) => {
    ...
    if (
      frameTimes.current.length > PERFORMANCE_CONFIG.PERFORMANCE_SAMPLE_SIZE
    ) {
    ...
    if (
      currentTime - lastPerformanceCheck.current >
      ...
      if (
        ...
        if (elapsed > 0) {
          ...
          if (
            PERFORMANCE_CONFIG.FRAME_RATE
              .EXCLUDE_FROM_PERFORMANCE_MEASUREMENT &&
...
function Scene({
  ...
  const enablePostProcessing =
    PERFORMANCE_CONFIG.POST_PROCESSING.ALWAYS_ENABLE ||
    performanceLevel === "high";
  ...
  useFrame((state, delta) => {
    ...
    if (
      ...
      const targetZ = 10; // Closer for mobile devices
    ...
    if (scrollY.current) {
      ...
      state.camera.position.y +=
        (targetY - state.camera.position.y) * lerpFactor;
    }
  ...
  const shapesConfig = performanceLevel === "high"
      ? backgroundShapes
      : performanceLevel === "medium"
        ? mediumPerformanceShapes
        : lowPerformanceShapes;
  ...
  return (
    <>
      ...
      <pointLight
        color={primaryColor}
        decay={0}
        intensity={performanceLevel === "low" ? 1.2 : 1.0}
        position={[0, 50, 0]}
      />
      ...
      {enablePostProcessing && (
        <EffectComposer enabled={enableBloom}>
          <Bloom
            mipmapBlur
            intensity={theme == "light" ? 0.5 : PERFORMANCE_CONFIG.POST_PROCESSING.BLOOM.INTENSITY}
            luminanceSmoothing={
              PERFORMANCE_CONFIG.POST_PROCESSING.BLOOM.SMOOTHING
            }
            luminanceThreshold={
              PERFORMANCE_CONFIG.POST_PROCESSING.BLOOM.THRESHOLD
            }
          />
        </EffectComposer>
      )}
...
const detectPerformanceLevel = (): PerformanceLevel => {
  ...
  const veryLowConcurrency =
    cpuCores < PERFORMANCE_CONFIG.DEVICE.FORCE_GRADIENT_CORES_MOBILE;
  ...
  const isVeryLowEnd =
    isMobile && veryLowConcurrency && lowMemory && !webGL2Support;
  ...
  // High-end detection
  const isHighEnd =
    (highConcurrency && webGL2Support && !isMobile) ||
...
const Background3DComponent = ({
  ...
  setBg3dVisible: (visible: boolean) => void;
}) => {
  ...
  const [entryAnimationStage, setEntryAnimationStage] = useState<
  ...
  const handlePerformanceChange = (fps: number) => {
    ...
    if (
      usingFrameLimiter &&
      fps >= PERFORMANCE_CONFIG.FRAME_RATE.TARGET_FPS * 0.9
    ) {
      return;
    }
    ...
    if (fps < PERFORMANCE_CONFIG.MIN_ACCEPTABLE_FPS) {
      ...
      // Segunda prueba: Si estamos en medio o ya intentamos una vez desde alto, cambiar a bajo
        ...
        (performanceLevel === "medium" || downgradeAttempts.current === 1) &&
        downgradeAttempts.current < 2
      ) {
        newLevel = "low";
      }
      // Último recurso: Si todo lo demás falló y hemos intentado el máximo de veces, usar gradiente
      else if (
        downgradeAttempts.current >= PERFORMANCE_CONFIG.MAX_DOWNGRADE_ATTEMPTS
      ) {
        newLevel = "gradient";
      }
  ...
  const shouldShowBackground3D =
    isLoaded && performanceLevel !== "gradient" && !webGLError;
  ...
  return (
    ...
    initial={{ opacity: 0, scale: 1.1 }}
      ...
      className={`fixed top-0 left-0 right-0 bottom-0 -z-10 w-full h-full transition-all`}
    >
      <Suspense>
